...
public class AdFoodCommand implements AdminCommandImpl{
	...
	@Autowired
	AdFoodDAO dao;
	public AdFoodCommand() {
		System.out.println("AdFoodCommand 빈 자동 생성 됨");
	}
	
	@Override
	public void execute(Model model) {
		System.out.println("AdFoodCommand -> execute() 호출");
		...
		ArrayList<AdFoodDTO> lists = dao.foodList(paramMap);
		
		//목록에 출력할 게시물의 가상 번호 계산 후 부여하기
		int virtualNum=0;
		int countNum=0;
		for(AdFoodDTO row : lists) {
