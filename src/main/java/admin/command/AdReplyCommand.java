...
import admin.model.AdReplyDTO;
import util.EnvFileReader;
import util.PagingUtil;

@Service
public class AdReplyCommand implements AdminCommandImpl{
	
	@Autowired
	AdReplyDAO dao;
	public AdReplyCommand() {
		System.out.println("AdReplyCommand 빈 자동 생성 됨");
	}
	
	@Override
	public void execute(Model model) {
		System.out.println("AdReplyCommand -> execute() 호출");
		...
		ArrayList<AdReplyDTO> lists = dao.replylist(paramMap);
		
		//목록에 출력할 게시물의 가상 번호 계산 후 부여하기
		int virtualNum=0;
		int countNum=0;
		for(AdReplyDTO row : lists) {
			...
			String pagingImg = PagingUtil.pagingImg(totalRecordCount,
					...
					req.getContextPath()+"/admin/reply.do?");
