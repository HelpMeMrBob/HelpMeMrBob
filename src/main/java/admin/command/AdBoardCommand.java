...
@Service
public class AdBoardCommand implements AdminCommandImpl{
	
	@Autowired
	AdBoardDAO dao;
	public AdBoardCommand() {
		System.out.println("AdBoardCommand 빈 자동 생성 됨");
	}
	
	@Override
	public void execute(Model model) {
		System.out.println("AdBoardCommand -> execute() 호출");
		...
		ArrayList<AdBoardDTO> lists = dao.boardlist(paramMap);
		
		//목록에 출력할 게시물의 가상 번호 계산 후 부여하기
		int virtualNum=0;
		int countNum=0;
		for(AdBoardDTO row : lists) {
			...
			String pagingImg = PagingUtil.pagingImg(totalRecordCount,
					...
					req.getContextPath()+"/admin/photo.do?");
