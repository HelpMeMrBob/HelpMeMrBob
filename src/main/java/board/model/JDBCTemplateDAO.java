	...
	//파일올리는 글쓰기
		...
		public void updateVisitCnt(final String idx)
		{
			...
			String sql="UPDATE board SET "
				...
				+" WHERE idx=? ";
			/*
			 행의 변화를 주는 쿼리문 실행이므로 update메서드를 사용함
			 첫번째인자는 쿼리문, 두번째 인자는 익명클래스를 통해 인파라미터를 설정한다.
			 */
		...
		public BoardDTO view(String idx) {
			...
			try {
				/*
				 ...
				 반환해야 한다. 그렇지 않으면 에러가 발생하게 되므로 예외처리를 하는 것이
				 봏다.
				 */
				dto=template.queryForObject(sql, 
						new BeanPropertyRowMapper<BoardDTO>(
								BoardDTO.class));
				/*
				 BeanPropertyRowMapper클래스는 쿼리의 실행결과를 DTO에 저장해주는 역할을 
		...
		//임시로 넣었음
		public int id(String idx, String id) {
			int retNum=0;
			String sql="SELECT*FROM board "
			...
			try {
				...
				예외를 발생시키기 때문이다.
				 */
				BoardDTO dto=template.queryForObject(sql, 
						new BeanPropertyRowMapper<BoardDTO>(
								BoardDTO.class));
				/*
		...
		public void delete(final String idx, final String id) {
		  ...
		  +"        SELECT * FROM board "; if(map.get("Word")!=null){ sql
		  +=" WHERE "+map.get("Column")+" " + " LIKE '%"+map.get("Word")+"%' "; } sql
